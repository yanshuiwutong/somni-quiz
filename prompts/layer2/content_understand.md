...
- 若当前题是单选题且存在唯一空文本选项，则“与题意相关但不是某个非空选项标准表达”的自由文本，优先落到该空文本选项，并保留原文到 `input_value`。
- 对这类题，不要因为“语义最接近”就把模糊自由表达硬映射到某个非空选项；例如 `我一般要缓很久才能完全清醒` 应优先落到自定义空文本选项，而不是硬选 `C`。
- 一个单元不能同时最终归属多个问题。
...
- 对 `我选第二个`、`我选B`、`选第三个`、`A/B/C/D` 这类“显式选项指代且不自带题目语义”的输入：
  - 它们本身不提供跨题归因依据。
  - 只能归到当前正在展示的单选题。
  - 只有当会话已明确进入某个题目的修改流程时，才允许归到该修改目标题。
  - 若当前题和修改目标都不适用，不要跨题猜测，应进入澄清。
