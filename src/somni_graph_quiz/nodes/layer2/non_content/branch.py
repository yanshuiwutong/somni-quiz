...
from somni_graph_quiz.tools import looks_like_weather_query


_PROMPTS_ROOT = Path(__file__).resolve().parents[5] / "prompts"
_VALID_NON_CONTENT_INTENTS = {
    "identity",
    "pullback_chat",
    "view_all",
    "view_previous",
...
class NonContentBranch:
    ...
    def run(self, graph_state: dict, turn_input: object) -> dict:
        non_content_intent = self._resolve_non_content_intent(graph_state, turn_input)
    ...
    def _resolve_non_content_intent(self, graph_state: dict, turn_input: object) -> str:
        raw_input = str(getattr(turn_input, "raw_input", "") or "").strip()
        control_action = detect_control_action(raw_input)
        if control_action is not None:
            return control_action
        if self._is_weather_query(graph_state, raw_input):
            return "weather_query"
        if "你是谁" in raw_input:
            return "identity"
        llm_intent = self._try_llm(graph_state, turn_input)
        if llm_intent in _VALID_NON_CONTENT_INTENTS:
            return llm_intent
        return "pullback_chat"

    def _is_weather_query(self, graph_state: dict, raw_input: str) -> bool:
        if looks_like_weather_query(raw_input):
            return True
        pending_weather_query = graph_state.get("session_memory", {}).get("pending_weather_query") or {}
        return bool(
            pending_weather_query.get("waiting_for_city")
            and looks_like_weather_city_followup(raw_input)
        )

    def _try_llm(self, graph_state: dict, turn_input: object) -> str | None:
        ...
        payload = {
            ...
            "pending_weather_query": graph_state.get("session_memory", {}).get("pending_weather_query"),
        }
        try:
            prompt_text = self._prompt_loader.render("layer2/non_content_intent.md", payload)
            output = invoke_json(
                provider,
                prompt_key="layer2/non_content_intent.md",
                prompt_text=prompt_text,
            )
        except Exception:
            return None
        candidate = str(output.get("non_content_intent", "")).strip()
        if candidate not in _VALID_NON_CONTENT_INTENTS:
            return None
        return candidate
