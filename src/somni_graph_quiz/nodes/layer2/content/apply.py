...
class ContentApplyNode:
    ...
    def run(
        ...
        if clarification_needed and not self._has_closed_units(content_units):
    ...
    def _has_closed_units(self, content_units: list[dict]) -> bool:
        return any(
            isinstance(unit, dict) and str(unit.get("winner_question_id") or "").strip()
            for unit in content_units
        )
