...
_COMPANION_SELECTOR_ONLY_REPLY_PATTERN = re.compile(
    r"^(?:我选)?(?:第\s*(?:\d+|十[一二三四五六七八九]?|[一二两三四五六七八九十])\s*(?:个|项)?|前者|后者|前一个|后一个)$"
)
...
class ContentUnderstandNode:
    ...
    def run(self, graph_state: dict, turn_input: object) -> dict:
        ...
        if self._is_selector_only_input(raw_input):
            if self._should_guard_companion_selector_only_input(session_memory, raw_input):
                self._log_diagnostic(
                    "selector_only_scope_guarded_for_companion_followup",
                    raw_input=raw_input,
                ...
                return {
                    ...
                    "clarification_question_title": None,
        ...
        if llm_output is not None:
            ...
            cleanup_before = {
                "content_units": list(understood.get("content_units", [])),
                "clarification_needed": bool(understood.get("clarification_needed", False)),
            }
            understood = self._drop_orphan_llm_tail_units_when_answer_exists(understood)
            cleanup_fields = self._build_orphan_tail_cleanup_diagnostic_fields(
                before_understood=cleanup_before,
                after_understood=understood,
            )
            if cleanup_fields is not None:
                self._log_diagnostic(
                    "llm_orphan_tail_cleanup_applied",
                    raw_input=raw_input,
                    **cleanup_fields,
                )
    ...
    def _drop_orphan_llm_tail_units_when_answer_exists(self, understood: dict) -> dict:
        if not bool(understood.get("clarification_needed", False)):
            return understood
        content_units = understood.get("content_units", [])
        if not isinstance(content_units, list) or not content_units:
            return understood

        has_closed_answer = any(
            isinstance(unit, dict)
            and str(unit.get("winner_question_id") or "")
            and self._content_unit_has_answer_signal(unit)
            for unit in content_units
        )
        if not has_closed_answer:
            return understood
        ...
        for unit in content_units:
            if not isinstance(unit, dict):
                ...
                continue
            has_winner = bool(str(unit.get("winner_question_id") or ""))
            has_candidates = bool(list(unit.get("candidate_question_ids", [])))
            has_answer_signal = self._content_unit_has_answer_signal(unit)
            if has_winner or has_candidates or has_answer_signal:
                kept_units.append(unit)
                continue
            dropped_units += 1
    ...
    def _build_orphan_tail_cleanup_diagnostic_fields(
        self,
        *,
        before_understood: dict,
        after_understood: dict,
    ) -> dict | None:
        before_units = before_understood.get("content_units", [])
        after_units = after_understood.get("content_units", [])
        before_clarification_needed = bool(before_understood.get("clarification_needed", False))
        after_clarification_needed = bool(after_understood.get("clarification_needed", False))
        before_unit_ids = self._content_unit_ids(before_units)
        after_unit_ids = self._content_unit_ids(after_units)
        dropped_unit_ids = [unit_id for unit_id in before_unit_ids if unit_id not in after_unit_ids]
        if (
            not dropped_unit_ids
            and before_clarification_needed == after_clarification_needed
            and len(before_unit_ids) == len(after_unit_ids)
        ):
            return None
        return {
            "before_clarification_needed": before_clarification_needed,
            "after_clarification_needed": after_clarification_needed,
            "before_content_unit_count": len(before_unit_ids),
            "after_content_unit_count": len(after_unit_ids),
            "dropped_unit_ids": dropped_unit_ids,
            "kept_unit_ids": after_unit_ids,
        }
    ...
    def _should_guard_companion_selector_only_input(self, session_memory: dict, raw_input: str) -> bool:
        companion_context = session_memory.get("companion_context") or {}
        if not bool(companion_context.get("active")):
            return False
        if not self._looks_like_companion_selector_only_reply(raw_input):
            return False
        return self._recent_turns_show_companion_open_followup(session_memory.get("recent_turns", []))

    def _looks_like_companion_selector_only_reply(self, raw_input: str) -> bool:
        normalized = _SELECTOR_ONLY_PUNCTUATION_PATTERN.sub("", str(raw_input).strip())
        if not normalized:
            return False
        return bool(_COMPANION_SELECTOR_ONLY_REPLY_PATTERN.fullmatch(normalized))

    def _recent_turns_show_companion_open_followup(self, recent_turns: object) -> bool:
        if not isinstance(recent_turns, list):
            return False
        for turn in reversed(recent_turns[-3:]):
            if not isinstance(turn, dict):
                continue
            if turn.get("assistant_mode") != "companion":
                continue
            if turn.get("assistant_followup_kind") == "open_followup":
                return True
        return False
    ...
    def _content_unit_ids(self, content_units: object) -> list[str]:
        if not isinstance(content_units, list):
            return []
        unit_ids: list[str] = []
        for unit in content_units:
